...
pub use f128_impl::x86::Avx;
#[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
#[cfg(feature = "nightly")]
#[cfg_attr(docsrs, doc(cfg(feature = "nightly")))]
pub use f128_impl::x86::Avx512;
...
impl FftSimdF128 for Avx {
    ...
    #[inline(always)]
    fn add(self, a: (Self::Reg, Self::Reg), b: (Self::Reg, Self::Reg)) -> (Self::Reg, Self::Reg) {
        let result = self._mm256_add_estimate_f128_f128(cast(a.0), cast(a.1), cast(b.0), cast(b.1));
        (cast(result.0), cast(result.1))
    }

    #[inline(always)]
    fn sub(self, a: (Self::Reg, Self::Reg), b: (Self::Reg, Self::Reg)) -> (Self::Reg, Self::Reg) {
        let result = self._mm256_sub_estimate_f128_f128(cast(a.0), cast(a.1), cast(b.0), cast(b.1));
        (cast(result.0), cast(result.1))
    }

    #[inline(always)]
    fn mul(self, a: (Self::Reg, Self::Reg), b: (Self::Reg, Self::Reg)) -> (Self::Reg, Self::Reg) {
        let result = self._mm256_mul_f128_f128(cast(a.0), cast(a.1), cast(b.0), cast(b.1));
        (cast(result.0), cast(result.1))
    }
}
...
impl FftSimdF128 for Avx512 {
    ...
    #[inline(always)]
    fn add(self, a: (Self::Reg, Self::Reg), b: (Self::Reg, Self::Reg)) -> (Self::Reg, Self::Reg) {
        let result = self._mm512_add_estimate_f128_f128(cast(a.0), cast(a.1), cast(b.0), cast(b.1));
        (cast(result.0), cast(result.1))
    }

    #[inline(always)]
    fn sub(self, a: (Self::Reg, Self::Reg), b: (Self::Reg, Self::Reg)) -> (Self::Reg, Self::Reg) {
        let result = self._mm512_sub_estimate_f128_f128(cast(a.0), cast(a.1), cast(b.0), cast(b.1));
        (cast(result.0), cast(result.1))
    }

    #[inline(always)]
    fn mul(self, a: (Self::Reg, Self::Reg), b: (Self::Reg, Self::Reg)) -> (Self::Reg, Self::Reg) {
        let result = self._mm512_mul_f128_f128(cast(a.0), cast(a.1), cast(b.0), cast(b.1));
        (cast(result.0), cast(result.1))
    }
}
        ...
        || {
            {
                ...
                while m < n / 4 {
                        ...
                        iter
                    {
                        ...
                        let iter =
                            izip!(z0_re0, z0_re1, z0_im0, z0_im1, z1_re0, z1_re1, z1_im0, z1_im1);
                        for (z0_re0, z0_re1, z0_im0, z0_im1, z1_re0, z1_re1, z1_im0, z1_im1) in iter
                        {
                            let (z0_re, z0_im) = ((*z0_re0, *z0_re1), (*z0_im0, *z0_im1));
                            let (z1_re, z1_im) = ((*z1_re0, *z1_re1), (*z1_im0, *z1_im1));
                            let (z1w_re, z1w_im) = simd.cplx_mul(z1_re, z1_im, w1_re, w1_im);
            ...
            // t = 2
            {
                ...
                ) in iter
                {
                    ...
                    let interleave = {
                        #[inline(always)]
                        |z0z0z1z1: [[f64; 4]; 2]| -> [[f64; 4]; 2] {
                            [
                                cast(simd.avx._mm256_permute2f128_pd::<0b00100000>(
                                    cast(z0z0z1z1[0]),
                                    cast(z0z0z1z1[1]),
                                )),
                                cast(simd.avx._mm256_permute2f128_pd::<0b00110001>(
                                    cast(z0z0z1z1[0]),
                                    cast(z0z0z1z1[1]),
                                )),
                            ]
                    ...
                    let splat2 = {
                        #[inline(always)]
                        |w: [f64; 2]| -> [f64; 4] {
                            let w00 = simd.sse2._mm_set1_pd(w[0]);
                            let w11 = simd.sse2._mm_set1_pd(w[1]);

                            let w0011 = simd.avx._mm256_insertf128_pd::<0b1>(
                                simd.avx._mm256_castpd128_pd256(w00),
                                w11,
                            );
                    ...
                    let w1_re = (splat2(*w1_re0), splat2(*w1_re1));
                    let w1_im = (splat2(*w1_im0), splat2(*w1_im1));

                    let [mut z0_re0, mut z1_re0] = interleave(*z0z0z1z1_re0);
                    let [mut z0_re1, mut z1_re1] = interleave(*z0z0z1z1_re1);
                    let [mut z0_im0, mut z1_im0] = interleave(*z0z0z1z1_im0);
                    let [mut z0_im1, mut z1_im1] = interleave(*z0z0z1z1_im1);
                    ...
                    *z0z0z1z1_re0 = interleave([z0_re0, z1_re0]);
                    *z0z0z1z1_re1 = interleave([z0_re1, z1_re1]);
                    *z0z0z1z1_im0 = interleave([z0_im0, z1_im0]);
                    *z0z0z1z1_im1 = interleave([z0_im1, z1_im1]);
                }
            }
                ...
                for (z0z1_re0, z0z1_re1, z0z1_im0, z0z1_im1, w1_re0, w1_re1, w1_im0, w1_im1) in iter
                {
                    ...
                    let permute = {
                        ...
                        |w: [f64; 4]| -> [f64; 4] {
                            let avx = simd.avx;
                            let w0123 = cast(w);
                            let w0101 = avx._mm256_permute2f128_pd::<0b00000000>(w0123, w0123);
                            let w2323 = avx._mm256_permute2f128_pd::<0b00110011>(w0123, w0123);
                            let w0213 = avx._mm256_shuffle_pd::<0b1100>(w0101, w2323);
                            cast(w0213)
                        }
                    };
                    ...
                    let interleave = {
                        #[inline(always)]
                        |z0z1: [[f64; 4]; 2]| -> [[f64; 4]; 2] {
                            [
                                cast(simd.avx._mm256_unpacklo_pd(cast(z0z1[0]), cast(z0z1[1]))),
                                cast(simd.avx._mm256_unpackhi_pd(cast(z0z1[0]), cast(z0z1[1]))),
                            ]
                        }
                    };

                    let w1_re = (permute(*w1_re0), permute(*w1_re1));
                    let w1_im = (permute(*w1_im0), permute(*w1_im1));

                    let [mut z0_re0, mut z1_re0] = interleave(*z0z1_re0);
                    let [mut z0_re1, mut z1_re1] = interleave(*z0z1_re1);
                    let [mut z0_im0, mut z1_im0] = interleave(*z0z1_im0);
                    let [mut z0_im1, mut z1_im1] = interleave(*z0z1_im1);
                    ...
                    *z0z1_re0 = interleave([z0_re0, z1_re0]);
                    *z0z1_re1 = interleave([z0_re1, z1_re1]);
                    *z0z1_im0 = interleave([z0_im0, z1_im0]);
                    *z0z1_im1 = interleave([z0_im1, z1_im1]);
                }
            }
        }
    });
                ...
                for (z0z1_re0, z0z1_re1, z0z1_im0, z0z1_im1, w1_re0, w1_re1, w1_im0, w1_im1) in iter
                {
                    ...
                    let permute = {
                        ...
                        |w: [f64; 2]| -> [f64; 8] {
                            ...
                            let w = cast(w);
                            let w01xxxxxx = avx512f._mm512_castpd128_pd512(w);
                            let idx = avx512f._mm512_setr_epi64(0, 0, 0, 0, 1, 1, 1, 1);

                            cast(avx512f._mm512_permutexvar_pd(idx, w01xxxxxx))
                        }
                    };
                    ...
                    let interleave = {
                        ...
                        |z0z0z1z1: [[f64; 8]; 2]| -> [[f64; 8]; 2] {
                            ...
                            [
                                cast(avx512f._mm512_permutex2var_pd(
                                    cast(z0z0z1z1[0]),
                                    idx_0,
                                    cast(z0z0z1z1[1]),
                                )),
                ...
                for (z0z1_re0, z0z1_re1, z0z1_im0, z0z1_im1, w1_re0, w1_re1, w1_im0, w1_im1) in iter
                {
                    ...
                    let permute = {
                        ...
                        |w: [f64; 4]| -> [f64; 8] {
                            let avx512f = simd.avx512f;
                            let w = cast(w);
                            let w0123xxxx = avx512f._mm512_castpd256_pd512(w);
                            let idx = avx512f._mm512_setr_epi64(0, 0, 2, 2, 1, 1, 3, 3);

                            cast(avx512f._mm512_permutexvar_pd(idx, w0123xxxx))
                        }
                    };
                ...
                for (z0z1_re0, z0z1_re1, z0z1_im0, z0z1_im1, w1_re0, w1_re1, w1_im0, w1_im1) in iter
                {
                    ...
                    let permute = {
                        ...
                        |w: [f64; 8]| -> [f64; 8] {
                            ...
                            cast(avx512f._mm512_permutexvar_pd(idx, w))
                        }
                    };
                    ...
                    let interleave = {
                        #[inline(always)]
                        |z0z1: [[f64; 8]; 2]| -> [[f64; 8]; 2] {
                            let avx512f = simd.avx512f;
                            [
                                cast(avx512f._mm512_unpacklo_pd(cast(z0z1[0]), cast(z0z1[1]))),
                                cast(avx512f._mm512_unpackhi_pd(cast(z0z1[0]), cast(z0z1[1]))),
                            ]
                        }
                    };
...
impl Plan {
    ...
    /// calling this function. When this function returns, the values in `buf_re0`, `buf_re1`, `buf_im0`, `buf_im1` will contain the
    /// terms of the forward transform in bit-reversed order.
    #[track_caller]
    pub fn fwd(
        &self,
        buf_re0: &mut [f64],
