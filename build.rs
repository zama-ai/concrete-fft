...
fn gen_generic_dif4_fn(n: usize, direction: &str) -> String {
...
fn gen_fn_ptr_array(direction: &str, arch: &str) -> String {
    let mut array = String::new();
    for i in 0..MAX_POW {
        let n = 1usize << i;
        array.push_str(&format!("{direction}fft_{n}_{arch}, "));
    }
    format!("[{array}]")
}

fn gen_runtime_fn_ptr(direction: &str, arch: &str) -> String {
    format!(
        "
...
fn write_dif4(out_dir: &OsStr) -> Result<()> {
    ...
    for i in 0..MAX_POW {
        ...
        for direction in DIRECTIONS {
            code.push_str(&format!("{}\n", gen_generic_dif4_fn(n, direction)));
            for arch in ARCHS {
                code.push_str(&format!("{}\n", gen_runtime_dif4_fn(n, direction, arch)));
            }
        }
    }
